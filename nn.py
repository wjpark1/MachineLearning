...
class Graph(object):
    ...
    def get_output(self, node):
        ...
        """
        "*** YOUR CODE HERE ***"
        nodeToFind = next(n for n in self.nodeStorage if n[0] == node)
        return nodeToFind[1]
    ...
    def add(self, node):
        ...
        self.nodeStorage.append([node])
        output = node.forward(self.get_inputs(node))
        addedNode = next(n for n in self.nodeStorage if n[0] == node)
        addedNode.append(output)
        addedNode.append(np.zeros_like(output))
    ...
    def backprop(self):
        ...
        "*** YOUR CODE HERE ***"
        nod = next(node for node in self.nodeStorage if node[0] == loss_node)
        nod[2] = 1.0
        for n in reversed(self.get_nodes()):
            back = n.backward(self.get_inputs(n), self.get_gradient(n))
            for i in range(0, len(n.get_parents())):
                nodeToUpdate = next(node for node in self.nodeStorage if node[0] == n.get_parents()[i])
                nodeToUpdate[2] = np.add(nodeToUpdate[2], back[i])
    ...
    def step(self, step_size):
        ...
        for n in self.get_nodes():
            if (n.__class__.__name__ == 'Variable'):
                nodeToUpdate = next(node for node in self.nodeStorage if node[0] == n)
                n.data = n.data - step_size*nodeToUpdate[2]
...
class ReLU(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        x = inputs[0]
        return [np.where(x < 0, 0, gradient)]
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        a = inputs[0]
        b = inputs[1]
        return [gradient * (a - b) / a.size, gradient * (b - a) / b.size]
