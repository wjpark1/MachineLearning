...
class RegressionModel(Model):
    ...
    def __init__(self):
        ...
        self.m = nn.Variable(1, 200)
        self.b = nn.Variable(200)
        self.m1 = nn.Variable(200,1)
    ...
    def run(self, x, y=None):
        ...
        """
        "*** YOUR CODE HERE ***"
        graph = nn.Graph([self.m, self.b, self.m1, self.b1])
        input_x = nn.Input(graph, x)
        xm_1 = nn.MatrixMultiply(graph, input_x, self.m)
        xm_plus_b_1 = nn.MatrixVectorAdd(graph, xm_1, self.b)
        hidden_output = nn.ReLU(graph, xm_plus_b_1)
        xm_2 = nn.MatrixMultiply(graph, hidden_output, self.m1)
        xm_plus_b_2 = nn.MatrixVectorAdd(graph, xm_2, self.b1)
        #get output associated with xm_plus_b_2, and return it in the else case below
        if y is not None:
            ...
            # added to the graph.
            "*** YOUR CODE HERE ***"
            #one more line where you add the node
            #you should try two layers, no for loop, just do the same calculation using
            #the previous output as the input for the next layer
            input_y = nn.Input(graph, y)
            loss = nn.SquareLoss(graph, xm_plus_b_2, input_y)
            return graph
        else:
            # At test time, the correct output is unknown.
            # You should instead return your model's prediction as a numpy array
            "*** YOUR CODE HERE ***"
            #just return the output, not the graph
            return graph.get_output(xm_plus_b_2)
...
class OddRegressionModel(Model):
    ...
    def run(self, x, y=None):
        ...
        negated_input_x = nn.Input(graph, x*-1)
        negated_xm_1 = nn.MatrixMultiply(graph, negated_input_x, self.m)
        negated_xm_plus_b_1 = nn.MatrixVectorAdd(graph, negated_xm_1, self.b)
        negated_hidden_output = nn.ReLU(graph, negated_xm_plus_b_1)
        negated_xm_2 = nn.MatrixMultiply(graph, negated_hidden_output, self.m1)
        negated_xm_plus_b_2 = nn.MatrixVectorAdd(graph, negated_xm_2, self.b1)

        negative_identity = nn.Input(graph, np.array([[-1.0]]))
        negated_final_output = nn.MatrixMultiply(graph, negated_xm_plus_b_2, negative_identity)
        odd_function = nn.Add(graph, xm_plus_b_2, negated_final_output)
        ...
        if y is not None:
            ...
            "*** YOUR CODE HERE ***"
            input_y = nn.Input(graph, y)
            #make node corresponding to negative identity matrix
            #multiply that with negated_xm_plus_b_2 using matrix_multiply function
            #use the add (NOT add_node) function defined in nn.py
            #use the result for the second argument to the call to SquareLoss below
            loss = nn.SquareLoss(graph, odd_function, input_y)
            return graph
        else:
            # At test time, the correct output is unknown.
            # You should instead return your model's prediction as a numpy array
            "*** YOUR CODE HERE ***"
            return graph.get_output(odd_function)
...
class DigitClassificationModel(Model):
    ...
    def __init__(self):
        ...
        self.m = nn.Variable(784, 200)
        self.b = nn.Variable(200)
        self.m1 = nn.Variable(200, 10)
    ...
    def run(self, x, y=None):
        ...
        if y is not None:
            "*** YOUR CODE HERE ***"
            input_y = nn.Input(graph, y)
            loss = nn.SoftmaxLoss(graph, xm_plus_b_2, input_y)
            return graph
        else:
            "*** YOUR CODE HERE ***"
            return graph.get_output(xm_plus_b_2)
...
class DeepQModel(Model):
    ...
    def __init__(self):
        ...
        self.b = nn.Variable(200)
        self.m1 = nn.Variable(200,2)
        self.b1 = nn.Variable(2)
    ...
    def run(self, states, Q_target=None):
        ...
        statesm_1 = nn.MatrixMultiply(graph, input_states, self.m)
        statesm_plus_b_1 = nn.MatrixVectorAdd(graph, statesm_1, self.b)
        hidden_output = nn.ReLU(graph, statesm_plus_b_1)
        statesm_2 = nn.MatrixMultiply(graph, hidden_output, self.m1)
        statesm_plus_b_2 = nn.MatrixVectorAdd(graph, statesm_2, self.b1)

        if Q_target is not None:
            "*** YOUR CODE HERE ***"
            input_q_target = nn.Input(graph, Q_target)
            loss = nn.SquareLoss(graph, statesm_plus_b_2, input_q_target)
            return graph
        else:
            "*** YOUR CODE HERE ***"
            return graph.get_output(statesm_plus_b_2)
...
class LanguageIDModel(Model):
    ...
    def __init__(self):
        ...
        self.W1 = nn.Variable(self.d, 400)
        self.W2 = nn.Variable(400, self.d)
        self.W3 = nn.Variable(self.d, 5)
        
        #ed
        
        self.h0 = nn.Variable(self.d)
        self.c = nn.Variable(self.num_chars, self.d)
    ...
    def run(self, xs, y=None):
        ...
        graph = nn.Graph([self.b1, self.b2, self.b3, self.W1, self.W2, self.W3, self.h0, self.c])
        
        #ed 
        H0 = nn.Input(graph, np.zeros((batch_size, self.d)))
        H = nn.MatrixVectorAdd(graph, H0, self.h0)
        # now you have H initialized
        
        for val in xs:
            input_c = nn.Input(graph, val)
            c = nn.MatrixMultiply(graph, input_c, self.c)
            h_update = nn.MatrixVectorAdd(graph, H, c)

            # NOW YOU HAVE A NODE WITH A MATRIX (batch_size x d)
            # PASTE YOUR USUAL ARCHITECURE AND MAKE SURE IT HAS AT LEAST ONE RELU NODE
            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            # MAKE SURE H AT THE END OF EACH LOOP IS (batch_size x d)
            # MAKE SURE H AT THE END OF EACH LOOP IS (batch_size x d)
            # MAKE SURE H AT THE END OF EACH LOOP IS (batch_size x d)
            # MAKE SURE H AT THE END OF EACH LOOP IS (batch_size x d)
            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

            xm_1 = nn.MatrixMultiply(graph, h_update, self.W1)
            xm_plus_b_1 = nn.MatrixVectorAdd(graph, xm_1, self.b1)
            hidden_output = nn.ReLU(graph, xm_plus_b_1)
            xm_2 = nn.MatrixMultiply(graph, hidden_output, self.W2)
            H = nn.MatrixVectorAdd(graph, xm_2, self.b2)
            
        # OK LAST STEP
        # DO SOME MATRIX TRANSFORMATIONS SO THAT H is (batch_size x 5)
        # USE THE VARIABLES YOU INITIALIZED IN PREVIOUS QUESTIONS
        # e.g.
        xm = nn.MatrixMultiply(graph, H, self.W3)
        x_m_2_plus_b = nn.MatrixVectorAdd(graph, xm, self.b3)
        "* YOUR CODE HERE *"
        
        if y is not None:
            input_y = nn.Input(graph, y)
            loss = nn.SoftmaxLoss(graph, x_m_2_plus_b, input_y)
            return graph
        else:
            return graph.get_output(x_m_2_plus_b)